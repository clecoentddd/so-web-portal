        ...
        return try {
            val response =
                    restTemplate.exchange(
                            ...
                            BoondProjectsResponse::class.java
                    )
            ...
            val managersById =
                    response.body?.included?.filter { it.typeResource == "users" }?.associate {
                        it.id to
                                Manager(
            ...
            val filteredProjects =
                    allProjects.filter { it.relationships.company?.data?.id == companyId }
            ...
            val projets =
                    filteredProjects.map {
                        ...
                        val managerFullName =
                                manager?.let { m -> "${m.firstName} ${m.lastName}".trim() }
                                        ?: "Unknown Manager"
