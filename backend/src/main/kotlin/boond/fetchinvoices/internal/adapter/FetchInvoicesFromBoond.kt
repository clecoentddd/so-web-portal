    ...
    return try {
      val response =
          restTemplate.exchange(
              url, HttpMethod.GET, HttpEntity<Any>(headers), BoondInvoicesResponse::class.java)
          ...
          allInvoices.filter { invoice ->
            ...
            val matchesProject =
                projectReference == null ||
                    invoice.relationships.project?.data?.reference == projectReference
            val matchesOrder =
                orderReference == null ||
                    invoice.relationships.order?.data?.reference == orderReference
